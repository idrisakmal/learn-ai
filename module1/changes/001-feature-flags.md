...
**Task:** none — task 3 committed · **Stage:** not started
...
- **Decide in task 4's PLAN where flags appear on screen.** The right pane is
  `ConfigurationPanel` today. Flags are a second list for the same Application,
  so the choice is a sibling panel below it, a tabbed pane, or a merged panel.
  ABOUT.md ranks "the admin UI is easy to use" first, so this is not a
  throwaway; settle it before writing components.
- **Extract `idParamSchema` to `lib/`.** Defined identically in four schema
  files now (applications, configurations, flags, and any resource that follows).
  Small, safe, and unrelated to any acceptance criterion — good filler between
  work items rather than inside one.
